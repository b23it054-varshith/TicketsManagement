...
export default function AdminPanel() {
  ...
  return (
    <div>
            ...
            <tbody>
              {filteredTickets.map(t => (
                <tr key={t._id}>
                  ...
                  <td>
                    <button className="btn btn-ghost btn-sm" id={`edit-ticket-${t._id}`}
                      onClick={() => setEditTicket({ ...t })}>Edit</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* Edit Ticket Modal */}
      <Modal isOpen={!!editTicket} onClose={() => setEditTicket(null)} title="Edit Ticket"
        footer={<><button className="btn btn-ghost" onClick={() => setEditTicket(null)}>Cancel</button>
          <button className="btn btn-primary" onClick={handleTicketUpdate}>Update Ticket</button></>}>
        {editTicket && (
          <div>
            <div style={{ marginBottom: 20, padding: '12px', background: 'var(--bg-hover)', borderRadius: 8 }}>
              <div style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>{editTicket.ticketId}</div>
              <div style={{ fontWeight: 600 }}>{editTicket.title}</div>
            </div>
            <div className="form-group">
              <label className="form-label">Status</label>
              <select className="form-control" value={editTicket.status} onChange={e => setEditTicket({ ...editTicket, status: e.target.value })}>
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={editTicket.priority} onChange={e => setEditTicket({ ...editTicket, priority: e.target.value })}>
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={editTicket.assignedTo?._id || ''} onChange={e => {
                const agent = users.find(u => u._id === e.target.value);
                setEditTicket({ ...editTicket, assignedTo: agent || null });
              }}>
